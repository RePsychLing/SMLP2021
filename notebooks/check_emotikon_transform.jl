...
# ╔═╡ 7e6953cf-f08f-43a9-a21a-5c20a4014b62
begin
	using  RCall, DataFrames, DataFrameMacros, StatsBase, MixedModels, Serialization
	using AlgebraOfGraphics, CairoMakie, MixedModelsMakie
	CairoMakie.activate!(type="svg")
	using PlutoUI
	TableOfContents()
end

# ╔═╡ 3d929ea6-1139-11ec-19b1-e70339e20752
md"""
# Transformed and Original Metrics in Emotikon

In the [Fühner et al. (2021, Sci Reps)](https://www.nature.com/articles/s41598-021-97000-4) the original metric of two tasks (Star, S20) is time, but they were transformed to speed scores in the publication prior to computing z-scores. The critical result is the absence of evidence for the age x Sex x Test interaction. Is this interaction significant if we analyse all tasks in their original metric?
...
# ╔═╡ e58001b9-405b-47c5-b469-53bdea462848
md"### Data and figure in publication"

# ╔═╡ 25758b68-522b-47b1-8b2a-e514ecb4ac05
begin	
	dat = rcopy(R"readRDS('./data/fggk21.rds')");
	@transform!(dat, :a1 = :age - 8.5);
	select!(groupby(dat, :Test), :, :score => zscore => :zScore);
	describe(dat)
end

# ╔═╡ 96d3e256-bf7f-409f-ba43-1392e3e85544
md"### Data and figure with z-scores based on original metric"

# ╔═╡ 097466e3-b073-42d4-83bb-f39f9515595f
begin	
	dat_om = rcopy(R"readRDS('./data/fggk21_om.rds')");
	@transform!(dat_om, :a1 = :age - 8.5);
	select!(groupby(dat_om, :Test), :, :score => zscore => :zScore);
	describe(dat_om)
end

# ╔═╡ 9cd2020b-7336-404e-a946-d916174e2d1c
md"## LMMs"

# ╔═╡ 2dd4c9d3-e4c2-4fae-bcaf-fe2a0e4832eb
md"### Contrasts"

# ╔═╡ 44069edc-b972-4400-8e8c-ead30b12ce92
begin
contr = merge(
...
# ╔═╡ 7f1a1f5e-8a44-4a18-b757-c2a4fe536a60
md"### Formula"

# ╔═╡ abc48a50-cdd1-4acc-ba74-cc74a592097b
f1 = @formula zScore ~ 1 + Test*a1*Sex +
     (1+Test+a1+Sex | School) + (1+Test | Child) + 
     zerocorr(1 + Test | Cohort);

# ╔═╡ cf5d3f94-292d-40ca-9faf-8927d00beffd
md"""
...
# ╔═╡ b62a39a1-fd62-4487-9e2c-a23518a613c2
begin
...
# ╔═╡ bc63e00f-dd73-4f53-8b00-1d8f8393413b
VarCorr(m1x)

# ╔═╡ eaeeae5f-7c8b-4d8d-8dca-2d3c990320cf
md"""
...
# ╔═╡ 718bde49-8f70-4bbc-b096-50ec1d7866c7
md" #### ... restoreoptsum!()"

# ╔═╡ 8e2f6be3-5010-43aa-b481-8d74d746bb92
begin
	m1_om = LinearMixedModel(f1, dat; contrasts=contr);
    restoreoptsum!(m1_om, "./fits/fggk21_m1_om_optsum.json");
end

# ╔═╡ 4dc9fdb1-a3ec-4839-b7df-c1a59d14a031
md" #### ... deserialize()"

# ╔═╡ 9153994d-7a65-4ad2-88f0-f5bb46ad0492
begin
  m1x_om = deserialize("./fits/m1_om.jls")
  m1x_om
end

# ╔═╡ 4f9c7104-10e9-4a06-8aa5-079421c676e8
VarCorr(m1x_om)

# ╔═╡ 32f39c3c-19ed-46ff-b190-e1cb3b752734
md"""
### Residual diagnostics for LMM m1
Residual plots for published LMM
"""

# ╔═╡ a158c365-a422-4d05-8022-8de2c6a7d095
#scatter(fitted(m1x), residuals(m1x)

# ╔═╡ 41f71e45-d1ce-4012-bf23-93f0ef22500d
 #qqnorm(m1x)

# ╔═╡ 2ec6e843-265b-47e0-b980-4420be4e2c72
md"""
### Residual diagnostics for LMM m1_om
Residual plots for LMM with Star and Speed in original metric.
"""

# ╔═╡ 24e4acf1-15fb-42d0-9888-77fc676e59fd
#scatter(fitted(m1_om_v2), residuals(m1_om_v2)

# ╔═╡ 2cc68650-cd3e-43c2-8b43-6718688b243c
#qqnorm(m1_om_v2)
...
git-tree-sha1 = "e2ae99b3b4675ddc91e9c421a775eff52a784696"
