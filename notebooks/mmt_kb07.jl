...
data(kb2bstrp.β) * mapping(:β; layout = :coefname) * AOG.density() |> draw

# ╔═╡ 4cbcdb5c-89a2-11eb-3522-c3645690afe2
data(filter(:column => ==(Symbol("(Intercept)")), DataFrame(kb2bstrp.σs))) * mapping(:σ, layout = :group) * AOG.density() |> draw

# ╔═╡ e7afbe94-89a6-11eb-1194-75349e639c70
DataFrame(kb2bstrp.σs)

# ╔═╡ c0b321e2-89a2-11eb-3af4-273b108abbf0
data(filter(:type => ==("ρ"), DataFrame(kb2bstrp.allpars))) * mapping(:value, layout = :names) * AOG.density() |> draw
