...
# ╔═╡ acbbdcab-b678-4143-95f2-d2be5f83e925
begin
	using CairoMakie
...
# ╔═╡ fbd6339a-10d4-11ec-3c2e-15dff05b3589
md"""
...
# ╔═╡ c72485ee-c4a7-427c-b590-5d9682fb09b3
sleepstudy = DataFrame(MixedModels.dataset(:sleepstudy))

# ╔═╡ 9dea9eac-ba97-49b1-9003-09142d186b43
m1 = fit(
	MixedModel,
...
# ╔═╡ ec752d1e-e14a-431a-82b8-32a6e1bd7b5f
md"""
...
# ╔═╡ 73e26ca3-51a4-4873-b561-f0b1e61879f1
m1.y

# ╔═╡ d366844b-7e04-49b8-8d49-0ef82274a7e5
m1.β

# ╔═╡ 0bbed5cf-1417-42ca-80f6-306200c50077
m1.X

# ╔═╡ 637d518b-dce3-40d5-87c8-bdc655570f8c
md"""
...
# ╔═╡ 2c1cfa14-99f9-4916-bdc4-8c8af3b34bc3
m1.b

# ╔═╡ 5fcb27d2-771c-417c-aa58-771cdd91a8ff
md"""
...
# ╔═╡ b8359d5d-0a2b-4bf4-a8d9-cb4ffe0d5b09
first(m1.reterms)

# ╔═╡ 93f478c1-f3fd-4163-954d-2d52a7e7efce
md"""
...
# ╔═╡ 21b519d9-2140-4af7-83c7-b851cdc18c18
md"""
In symbols we would write the *linear predictor expression* as
```math
\eta = {\bf X\beta + Z b}
```
where $\eta$ has 180 elements, $\beta$ has 2 elements, $\bf b$ has 36 elements, $\bf X$ is of size 180 × 2 and $\bf Z$ is of size 180 × 36.

For a linear model or linear mixed model the linear predictor **is** the mean response, $\mu$.
...
\end{aligned}
```
where $\Sigma_\theta$ is a 36 × 36 symmetric covariance matrix that has a special form - it consists of 18 diagonal blocks, each of size 2 × 2 and all the same.

Recall that this symmetric matrix can be constructed from the parameters θ, which generate the lower triangular matrix λ, and the estimate σ̂².
"""

# ╔═╡ 765baad6-b96f-4d13-b46c-bd848d8845c4
m1.θ

# ╔═╡ 7a2a3c47-44e1-4061-a186-7e3a9b0269be
λ = only(m1.λ)  # with multiple grouping factors there will be multiple λ's

# ╔═╡ cd024e2b-592a-4801-bfa0-1a58e3dded1e
Σ = varest(m1) * (λ * λ')

# ╔═╡ bce7177c-e4d9-4acf-9ac1-2ff8f244c038
md"Compare the diagonal elements to the *Variance* column of"
...
> The conditional distribution of the response vector, $\mathcal Y$, given that the random effects vector, $\mathcal B =\bf b$, is a multivariate normal (or *Gaussian*) distribution whose mean, $\mu$, is the linear predictor, $\eta=\bf X\beta+Zb$, and whose covariance matrix is $\sigma^2\bf I$.  That is, conditional on $\bf b$, the elements of $\mathcal Y$ are independent normal random variables with constant variance, $\sigma^2$, and means of the form $\mu = \eta = \bf X\beta+Zb$.
