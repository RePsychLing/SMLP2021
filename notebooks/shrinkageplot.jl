...
# ╔═╡ 6dff0c66-04f4-48af-8916-5a1b3332e581
begin
	using CairoMakie
	using DataFrames
	using LinearAlgebra
...
# ╔═╡ 222d84da-1099-11ec-3b7e-39bb2e7346c3
md"""
# More on the shrinkage plots
...
# ╔═╡ f20eb94c-f625-4e25-b270-0f95e29afdb4
TableOfContents()

# ╔═╡ e5166199-25db-4cbd-b5de-3b6800bd2a37
md"Load the kb07 data set (don't tell Reinhold that I used these data)."

# ╔═╡ f33adf3d-5d57-4b3c-aaa4-0ec837f57fc0
...
md"Notice the zeros on the diagonal.  A triangular matrix with zeros on the diagonal is singular."

# ╔═╡ 20a9bfe0-75b9-4f4b-9e5d-7cae25dcaa51
l2 = last(m1.λ)    # this one is not singular

# ╔═╡ e497c64d-cf01-4463-8cf0-2de9ed7608d8
md"To regenerate the covariance matrix we need to know that the covariance is not the *square* of `l1`, it is `l1 * l1'` (so that the result is symmetric) and multiplied by σ̂²"

# ╔═╡ dd029a5e-a56c-4383-9814-8477651b4a9c
Σ₁ = varest(m1) .* (l1 * l1')

# ╔═╡ 7744012d-713f-4817-89d7-293631562986
diag(Σ₁)  # compare to the variance column in the VarCorr output

# ╔═╡ a191d74f-329a-410c-aa21-10f694254cc1
sqrt.(diag(Σ₁))

# ╔═╡ 65ef23a0-eb47-4989-8d4e-ccc8dd164562
md"""
## Shrinkage plots
"""
...
m2 = fit(MixedModel,
	@formula(rt_trunc ~ 1 + prec*spkr*load + (1|subj) + (1+prec|item)),
	kb07;
	contrasts=Dict(
		:subj => Grouping(),
		:item => Grouping(),
...
# ╔═╡ fc822911-0505-46ca-9dc3-d7396d4c35cb
shrinkageplot(m2)

# ╔═╡ 9b595e88-4b63-42e3-a627-1e12acb9f31c
m3 = fit(MixedModel,
	@formula(rt_trunc ~ 1 + prec + spkr + load + (1|subj) + (1+prec|item)),
	kb07;
	contrasts=Dict(
...
# ╔═╡ 5678d363-55c7-4f30-bdb9-68a57d615ed4
VarCorr(m3)

# ╔═╡ 13d1d407-3df5-4b0e-a515-cffe1b6227ff
